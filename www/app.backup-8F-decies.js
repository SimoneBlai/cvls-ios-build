...
function bindEvents() {
  ...
  bind("openBollaturaBtn", "click", () => {
    if (window.CvlsGeobollatura && typeof window.CvlsGeobollatura.init === "function") {
      window.CvlsGeobollatura.init();
    }
  });
  bind("openAdminBollatureBtn", "click", openAdminBollature);
  
  bind("sideOpenRegistroPresenzeBtn", "click", () => cvlsShowPage("pageRegistroPresenze", "Registro Presenze"));
  ...
  try {
    ...
    if (user) {
      ...
      if (currentUserProfile) {
        ...
        localStorage.setItem("cvls_user_name", currentUserProfile.nome_tecnico);
...
function initRegistroPresenzeLuogoSearch() {
  ...
  function syncRegistroPresenzeActiveLuogo() {
    ...
    active.cittaNome = presidiArr.length > 0
      ? presidiArr.map(function (p) { return p.NomePresidio; }).filter(Boolean).join(", ")
      : null;

    active.cantiereNome = ubicazioniArr.length > 0
